...
class ExpertEnsemble:
    ...
    def update_all(self, X, y):
        """
        Batch train all experts once on full dataset.
        Each expert gets a random subsample (bagging) so they learn
        different aspects of the data — this is what makes the ensemble strong.
        """
        X = np.array(X, dtype=np.float32)
        y = np.array(y, dtype=np.float32)
        n = len(X)
        for expert in self.experts:
            # each expert trains on a different 70% subsample (bagging)
            idx = np.random.choice(n, size=int(n * 0.7), replace=False)
            expert.update(X[idx], y[idx])
