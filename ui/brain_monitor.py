...
class BrainMonitorWidget(QWidget):
    ...
    def update_state(self):
        ...
        self._update_session_signals(state)
    ...
    def _update_session_signals(self, state):
        symbols = state.get('symbols', [])
        session = state.get('session', '---')
        is_weekend = state.get('is_weekend', False)
        progress = state.get('bootstrap_progress', 0.0)
        ...
        if is_weekend:
            regime_text = '<span style="color:#d29922">⏸ WEEKEND — Market Closed</span>'
        else:
            regime_text = f'<span style="color:#3fb950">{session}</span>'
        self.signal_labels['Current Regime'].setText(regime_text)

        if not is_trained:
            pct = int(progress * 100)
            train_text = f'<span style="color:#58a6ff">Bootstrapping {pct}%...</span>'
        else:
            train_text = '<span style="color:#3fb950">✅ Trained on history</span>'
        self.signal_labels['Learning Rate'].setText(train_text)
